...
def traducir_oracion(oracion):
    ...
    for palabra in palabras:
        if palabra.lower() in diccionario:
            # Si la palabra está en el diccionario, la añadimos a la lista
            oracion_traducida.append(diccionario[palabra.lower()])
            letras_a_mover.append(diccionario[palabra.lower()])
        else:
            oracion_traducida.append(palabra)

    # Si hay letras a mover, las reubicamos en la segunda posición
    if letras_a_mover:
        # Eliminamos las letras que se moverán
        for letra in letras_a_mover:
            oracion_traducida.remove(letra)
        # Insertamos las letras en la segunda posición
        for letra in letras_a_mover:
            oracion_traducida.insert(1, letra)
